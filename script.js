...
document.getElementById("lexicalBtn").addEventListener("click", function () {
    ...
    const lexicalError = /[^a-zA-Z0-9\s{}()=+\-*/.;:,_&|!<>^%$#@'"\[\]`]/.test(code);

    if (lexicalError) {
        displayMessage("Lexical analysis failed! Invalid characters found.", "lightcoral"); // Display error message if invalid characters found
    } else {
...
document.getElementById("semanticBtn").addEventListener("click", function () {
    ...
    // const declarationPattern = /\b(int|float|double|char|boolean|long|short|byte)\s+([a-zA-Z_][a-zA-Z0-9_]*)\b/g;
    const declarationPattern = /\b(int|float|double|char|boolean|long|short|byte|String)\s+([a-zA-Z_][a-zA-Z0-9_]*)\b/g;
    const declaredVars = [];
    let match;

    while ((match = declarationPattern.exec(code)) !== null) {
        declaredVars.push(match[2]); // Collect declared variables
    }

    const identifierPattern = /\b([a-zA-Z_][a-zA-Z0-9_]*)\b/g;
    const undeclaredVars = [];
    const stringsAndCommentsPattern = /(["'].*?["'])|(\/\/.*$|\/\*[\s\S]*?\*\/)/gm;
    const sanitizedCode = code.replace(stringsAndCommentsPattern, ""); // Remove strings and comments

    while ((match = identifierPattern.exec(sanitizedCode)) !== null) {
        const varName = match[1];
        if (javaKeywords.includes(varName) || declaredVars.includes(varName)) {
            continue;
        }
        if (!undeclaredVars.includes(varName)) {
            undeclaredVars.push(varName); // Collect undeclared variables
        }
    }

    // If undeclared variables are found, display error message
    if (undeclaredVars.length > 0) {
        result.value = "Semantic analysis failed! Undeclared variable(s) found: " + undeclaredVars.join(", ");
